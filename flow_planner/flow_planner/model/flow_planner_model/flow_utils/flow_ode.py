...
class FlowODE(Scheduler):
    ...
    def generate(self, x_init, model_fn, model_pred_type, use_cfg, cfg_weight=None, **model_extra):
        ...
        Note: ``cfg_weight`` is now an explicit kwarg. Before this fix the
        caller's ``cfg_weight`` was silently absorbed into ``**model_extra``
        and then forwarded through the ODE solver as a junk kwarg into the
        decoder — the actual CFG mixing weight was always whatever the YAML
        stored at instantiation time (``self.cfg_weight``). When ``cfg_weight``
        is None we fall back to ``self.cfg_weight`` for backwards compatibility.
        '''
        ...
        velocity_model = VelocityModel(model_fn, self.path, velocity_func, use_cfg=use_cfg, cfg_weight=effective_cfg_weight)
