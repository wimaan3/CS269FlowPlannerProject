...
class BiasedAttention(Module):
    ...
    def forward(
        ...
        *,
        ...
        # When enable_attn_dist=False (audit Patch 5 for Frenet), JointAttention
        # sets gen_taus=None, which makes taus=None. The upstream decoder still
        # forwards model_extra['token_dist'] as attn_dist, so attn_bias arrives
        # non-None. Both are required for the modulation; without taus the
        # multiplication on the next line is None * Tensor → TypeError. Guard
        # on both so disabling the attn-distance bias actually disables it.
        if attn_bias is not None and taus is not None:
